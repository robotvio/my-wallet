...
function getRandomObject(walletId: string,walletAddress: string): MockObjectETH {
    ...
    const feeExchangeRateUnit = Math.random() < 0.5?'USDT':'ETH'
    const feeSymbol = feeExchangeRateUnit==='USDT'?'ETH':'USDT'
    ...
    const exchangeRateUnit = Math.random() < 0.5?'USDT':'ETH'
    const symbol = exchangeRateUnit==="USDT"?"ETH":"USDT"
