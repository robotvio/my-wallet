...
export const getTransactionsBTCFake = async (walletId:string,walletAddress:string) :Promise<any[]> => {
    const res = await fetch(`http://localhost:3000/api/fake/btc/?walletAddress=${walletAddress}&walletId=${walletId}`, {
        method: 'GET',
    })
    return new Promise<any[]>(async (resolve, reject) => {
...
export const getTransactionsETHFake = async (walletId:string,walletAddress:string) :Promise<any[]> => {
    const res = await fetch(`http://localhost:3000/api/fake/eth/?walletAddress=${walletAddress}&walletId=${walletId}`, {
        method: 'GET',
    })
    return new Promise<any[]>(async (resolve, reject) => {
